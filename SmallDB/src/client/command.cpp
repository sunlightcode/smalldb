...
int HelpCommand::execute( ossSocket & sock, std::vector<std::string> & argVec )
{
   ...
   printf("%s [server] [port]-- connecting smalldb server\n", COMMAND_CONNECT);
   printf("%s -- sending a insert command to smalldb server\n", COMMAND_INSERT);
   printf("%s -- sending a query command to smalldb server\n", COMMAND_QUERY);
   printf("%s -- sending a delete command to smalldb server\n", COMMAND_DELETE);
   printf("%s [number]-- sending a test command to smalldb server\n", COMMAND_TEST);
